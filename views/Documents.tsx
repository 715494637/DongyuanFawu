...
import { cachedApi, api } from '../services/apiService';
...
const Documents: React.FC = () => {
    ...
    const fetchDocs = async () => {
      try {
        ...
        const [docsData, catsData] = await Promise.all([
          ...
          cachedApi.getDocCategories().catch(() => ["全部", "前介承接", "违规整改", "内部管理", "风险防范", "催收增收"])
        ]);
  ...
  // 默认分类（仅作为 fallback）
  const defaultCategories = ["全部", "前介承接", "违规整改", "内部管理", "风险防范", "催收增收"];

  // 使用 API 获取的分类列表，如果没有则使用默认值
