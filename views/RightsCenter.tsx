  ...
  const refreshData = async () => {
    try {
      ...
      const transformedLevels = (Array.isArray(levelsData) ? levelsData : []).map((apiLevel: any) => ({
        ...
        selectable_projects_count: apiLevel.selectable_projects_count ?? apiLevel.selectableProjectsCount ?? 0,
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full bg-[#0F1115] items-center justify-center">
        <div className="animate-spin text-[#FFD700]"><Gem size={32} /></div>
        <p className="text-slate-400 text-xs mt-2">加载权益中心...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-[#0F1115] animate-fade-in relative overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-2 relative z-10">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#FFD700] via-[#FDB931] to-[#D4AF37]">权益中心</h1>
        </div>
      ...
      <div className="px-6 py-4 flex gap-3 overflow-x-auto no-scrollbar relative z-10">
        {levels.map(lvl => {
          ...
          const btnClass = isActive
            ? 'flex-1 min-w-[100px] py-3 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all relative overflow-visible bg-gradient-to-b from-yellow-500/20 to-yellow-500/5 border-yellow-500/50 shadow-[0_0_15px_rgba(255,215,0,0.15)]'
            : 'flex-1 min-w-[100px] py-3 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all relative overflow-visible bg-white/5 border-white/10 text-slate-500';
          const textClass = isActive ? 'text-sm font-black italic text-yellow-500' : 'text-sm font-black italic text-slate-400';
          return (
            <button key={lvl.id} onClick={() => setActiveTab(lvl.id)} className={btnClass}>
              <span className={textClass}>{lvl.level_name}</span>
              <span className="text-[9px] uppercase opacity-70 tracking-widest">{lvl.level_code}</span>
              {!isUnlocked && <div className="absolute top-1 right-1 text-slate-600"><Lock size={10} /></div>}
              {isCurrent && <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap z-20">CURRENT</div>}
            </button>
          );
        })}
      </div>
      ...
      <div className="flex-1 px-4 pb-24 overflow-y-auto no-scrollbar relative z-10">
        {activeLevel && (
          <div className={status === 'UNLOCKED'
            ? 'rounded-[2rem] p-1 border border-yellow-500/30 bg-gradient-to-b from-yellow-500/30 to-transparent'
            : 'rounded-[2rem] p-1 border border-slate-700/30 bg-gradient-to-b from-slate-700/30 to-transparent'
          }>
            <div className="bg-[#181A20] rounded-[1.8rem] p-6 min-h-[400px]">
              <div className="flex justify-between items-start mb-6 pb-6 border-b border-white/5">
                <div>
                  {/* 等级标题 */}
                  <h2 className={status === 'UNLOCKED' ? 'text-xl font-black tracking-wide text-white' : 'text-xl font-black tracking-wide text-slate-500'}>
                  ...
                  {(() => {
                    ...
                    return (
                      <p className={`text-xs mt-2 leading-relaxed ${status === 'UNLOCKED' ? 'text-slate-300' : 'text-slate-500'}`}>
                        {benefits.desc}
                      </p>
                    );
                  })()}
                  {status === 'LOCKED' && (
                    <p className="text-xs text-orange-500 mt-2 font-bold flex items-center gap-1 animate-pulse">
                      <Lock size={12} />累计委托数据达到 {(activeLevel.min_amount/10000).toFixed(0)} 万元解锁
                    </p>
                  )}
                  {status === 'UNLOCKED' && (
                    <p className="text-xs mt-2 font-bold flex items-center gap-1" style={{ color: GOLD }}>
                      <CheckCircle2 size={12} />当前等级权益已生效
                    </p>
                  )}
                </div>
                <div style={status === 'UNLOCKED'
                  ? { backgroundColor: GOLD_TRANSPARENT_10, color: GOLD, padding: '12px', borderRadius: '9999px' }
                  : { backgroundColor: SLATE_700, color: SLATE_600, padding: '12px', borderRadius: '9999px' }
                }>
                  <Gem size={28} />
                </div>
              </div>
              ...
              <div className="space-y-6">
                <h3 className="font-black text-sm text-white flex items-center gap-2">
                  <Briefcase size={16} style={{ color: GOLD }} />常规法律顾问服务
                </h3>
                <div className="space-y-4">
                  {(() => {
                    const benefits = getLevelBenefits(activeLevel.level_name, activeLevel.level_code);
                    return benefits.services.map((item, idx) => (
                      <div key={idx} className={status === 'UNLOCKED' ? 'relative pl-4 border-l border-yellow-500/50' : 'relative pl-4 border-l border-slate-700'}>
                        <h4 className={status === 'UNLOCKED' ? 'text-sm font-bold mb-1 text-slate-200' : 'text-sm font-bold mb-1 text-slate-500'}>
                          {idx + 1}. {item.title}
                        </h4>
                        {item.desc && (
                          <p className={`text-xs mt-1 leading-relaxed ${status === 'UNLOCKED' ? 'text-slate-400' : 'text-slate-500'}`}>{item.desc}</p>
                        )}
                      </div>
                    ));
                  })()}
              ...
              {activeLevel.min_amount >= 0 && (
                <div className="mt-8 pt-6 border-t border-white/5">
                  <div className="flex justify-between items-end mb-4">
                    <h3 className={status === 'UNLOCKED' ? 'font-black text-sm text-white flex items-center gap-2' : 'font-black text-sm text-slate-500 flex items-center gap-2'}>
                      <Star size={16} className={status === 'UNLOCKED' ? 'text-yellow-500' : 'text-slate-600'} />专项法律服务定制
                    </h3>
                    {status === 'UNLOCKED' && (
