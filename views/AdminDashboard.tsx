...
const AdminDashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const refreshData = async () => {
    const token = getToken();
    try {
      const [docsRes, risksRes, evidenceRes, lawsRes, scriptsRes, sopsRes, specialRes, requestsRes, usersRes, enterprisesRes] = await Promise.all([
        api.getDocuments().catch(() => []),
        api.getRisks().catch(() => []),
        api.getEvidence().catch(() => []),
        api.getCivilCode().catch(() => []),
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-slate-300 flex font-sans overflow-hidden">
      ...
      {editingDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-black/80 backdrop-blur-sm animate-fade-in">
          <form onSubmit={handleSaveDoc} className="bg-[#12151c] w-full max-w-lg rounded-[2rem] border border-slate-800 p-8 shadow-2xl">
            <h3 className="font-black text-white text-xl mb-6">文档编辑</h3>
            <div className="space-y-4">
              ...
              <div><label className="text-xs text-slate-500 font-bold">分类</label><input value={editingDoc.category} onChange={e => setEditingDoc({ ...editingDoc, category: e.target.value })} className="w-full bg-black/40 border border-slate-800 rounded-xl py-3 px-4 text-sm text-white" /></div>
