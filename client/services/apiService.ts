...
// 统一的错误处理函数
const handleApiError = async (response: Response, isLoginRequest: boolean = false) => {
  // 如果响应状态是401
  if (response.status === 401) {
    // 如果是登录请求，说明是账号或密码错误
    if (isLoginRequest) {
      const errorData = await response.json().catch(() => ({}));
      const error = new Error(errorData.detail || errorData.message || '账号或密码错误');
      throw error;
    }

    // 否则说明Token无效或过期，清除本地存储的token
  ...
  if (!response.ok) {
    ...
    if (Array.isArray(errorData.detail)) {
      // Pydantic 验证错误：提取所有错误消息
      errorMessage = errorData.detail
        .map((err: any) => `${err.loc?.[1] || '字段'}: ${err.msg}`)
        .join('; ');
    } else {
      errorMessage = errorData.detail || errorData.message || `请求失败: ${response.status}`;
    }
...
// 封装fetch请求，添加统一错误处理
const fetchWithErrorHandler = async (url: string, options?: RequestInit, isLoginRequest: boolean = false) => {
  const response = await fetch(url, options);
  return handleApiError(response, isLoginRequest);
};
...
export const api = {
  ...
  async login(username: string, password: string) {
    ...
    if (!response.ok) {
      return handleApiError(response, true); // 标记为登录请求
    }
  ...
  // 创建文档分类
  async createDocCategory(category: string, token: string) {
    return fetchWithErrorHandler(`${API_BASE}/documents/categories`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  // 删除文档分类
  async deleteDocCategory(category: string, token: string) {
    return fetchWithErrorHandler(`${API_BASE}/documents/categories/${encodeURIComponent(category)}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token}` }
    });
  },
  ...
  // ============ 语音转文字 ============
  async speechToText(audioBlob: Blob): Promise<{ text: string; error?: string }> {
