...
export const CacheProvider: React.FC<CacheProviderProps> = ({ children }) => {
  ...
  const invalidateUser = useCallback((): void => {
    cacheRef.current.delete(CACHE_KEYS.USER_INFO);
    cacheRef.current.delete(CACHE_KEYS.ENTERPRISE_STATS);

    // 发布缓存失效事件
    window.dispatchEvent(new CustomEvent('cache-invalidated', {
      detail: { keys: [CACHE_KEYS.USER_INFO, CACHE_KEYS.ENTERPRISE_STATS] }
    }));
