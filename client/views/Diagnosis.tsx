...
import { ChevronRight, FileText, Sparkles, ArrowLeft, Shield, AlertTriangle, HeadphonesIcon, Camera, MessageSquare, Copy, Mic, MicOff, Loader2 } from 'lucide-react';
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          if (result.text) {
            setUserInput(prev => prev ? prev + ' ' + result.text : result.text);
          }
        } catch (e: any) {
          alert('语音识别失败：' + (e.message || '请重试'));
        } finally {
    ...
    <div className="p-6 space-y-6 animate-fade-in pb-24">
      ...
      <div className="flex gap-3">
        <button
          ...
          className={"flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm transition-all active:scale-95 " + (isRecording ? "bg-red-500 text-white animate-pulse" : "bg-white border border-gray-200 text-gray-500")}
        ...
        <button
          ...
          className="flex-1 bg-slate-900 text-white py-5 rounded-2xl font-black text-sm flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all disabled:opacity-50"
        >
