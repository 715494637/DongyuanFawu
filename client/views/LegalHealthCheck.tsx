...
3. 企业抗风险能力强，能够有效规避大部分日常法律陷阱。`,
  ...
  const generateAIReport = async () => {
      ...
      let fullInput = "";
      sections.forEach((sec, sIdx) => {
          fullInput += `【${sec.section_title}】\n`;
          sec.questions.forEach((q, qIdx) => {
              const answerKey = `${sIdx}-${qIdx}`;
              const userAnswer = answers[answerKey] === 0 ? "是/有" : "否/不清楚";
              fullInput += `  问题：${q.question}\n`;
              fullInput += `  回答：${userAnswer}\n`;
          });
          fullInput += "\n";
      });

      const prompt = `你是一位资深物业法律顾问。以下是用户完成的企业法务体检详情：

用户得分（风险指数）：${score} 分
系统评级：${resultStats.level}

${fullInput}请生成一份深度的《企业合规诊断报告》，必须包含：
1. 【总体风险评级】：根据用户的回答和得分，明确指出风险等级（高/中/低），并给出简短的专业评语。
2. 【核心法律风险分析】：针对用户回答中标记为"否/不清楚"的问题，引用《民法典》、《物业管理条例》或相关司法解释，详细说明可能导致的法律后果。
3. 【整改行动指南】：给出3-5条具体的、可落地的整改建议。
4. 【东元工具推荐】：推荐使用"东元法物"系统内的工具或建议申请专项服务。
