...
const CollectionHelper: React.FC = () => {
  ...
  const handleAddDebtor = async () => {
    ...
    const newRec = {
      debtor_name: newName,
      property_unit: newRoom,
      arrears_amount: newAmount
    };
  ...
  const generateLetter = (type: string) => {
    ...
    if (type === 'REMINDER') {
        text = `【温馨提示】尊敬的${selectedDebtor.owner_name || selectedDebtor.debtor_name || selectedDebtor.ownerName}业主（房号${selectedDebtor.room_number || selectedDebtor.roomNumber || selectedDebtor.property_unit}）：您好。截至${date}，您暂欠物业费${amountStr}。物业服务是维持小区运转的基础，请您尽快缴纳。`;
    } else if (type === 'LETTER') {
        text = `【律师催款函】\n致${selectedDebtor.owner_name || selectedDebtor.debtor_name || selectedDebtor.ownerName}：\n本律师受东元物业委托，就您拖欠${selectedDebtor.room_number || selectedDebtor.roomNumber || selectedDebtor.property_unit}物业费共计${amountStr}一事，郑重致函...\n请于3日内付清，否则将启动司法程序。`;
    } else {
        text = `【支付令申请书草稿】\n申请人：东元物业\n被申请人：${selectedDebtor.owner_name || selectedDebtor.debtor_name || selectedDebtor.ownerName}\n请求事项：支付物业费${amountStr}...`;
    }
        ...
        <div className="bg-white p-6 pb-2 shadow-sm shrink-0">
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4">
                ...
                <div className="bg-blue-50 p-4 rounded-2xl min-w-[140px] border border-blue-100">
                    ...
                    <div className="text-2xl font-black text-blue-600 mt-1">¥ {(debtors.reduce((sum, d) => sum + parseFloat(d.amount || d.arrears_amount || '0'), 0) / 10000).toFixed(1)} <span className="text-xs">万</span></div>
                </div>
            </div>
        </div>
                    ...
                    <div key={d.id} onClick={() => { setSelectedDebtor(d); setShowCalc(false); setCalcResult(null); }} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm active:scale-[0.98] transition-all relative">
                        <div className="flex justify-between items-start">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="font-bold text-slate-800">{d.room_number || d.roomNumber || d.property_unit} {d.owner_name || d.debtor_name || d.ownerName}</h4>
                                    <p className="text-xs text-slate-400 mt-0.5">欠费: <span className="text-red-500 font-bold">¥{parseFloat(d.amount || d.arrears_amount || '0')}</span></p>
                                </div>
                            </div>
            ...
            <div className="fixed inset-0 z-50 bg-slate-50 flex flex-col animate-fade-in-up">
                <div className="bg-white p-6 shadow-sm flex items-center gap-4 sticky top-0 z-10">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">{selectedDebtor.room_number || selectedDebtor.roomNumber || selectedDebtor.property_unit} · 欠费 ¥{selectedDebtor.amount || selectedDebtor.arrears_amount || '0'}</p>
                    </div>
                </div>
