...
interface LayoutProps {
  ...
  previousViewRef?: React.MutableRefObject<ViewState | null>;
}

const Layout: React.FC<LayoutProps> = ({ currentView, setCurrentView, children, scrollRef, previousViewRef }) => {
          ...
          {showBackButton ? (
            <>
              <button
                onClick={() => {
                  // "从哪来回哪去"导航逻辑
                  const referrer = previousViewRef?.current;
                  if (referrer && mainTabs.includes(referrer)) {
                    // 如果来源是一级页面，直接返回
                    setCurrentView(referrer);
                    if (previousViewRef) previousViewRef.current = null;
                  } else if (toolboxChildren.includes(currentView)) {
                    // 如果当前页面是工具箱子页面，返回工具箱
